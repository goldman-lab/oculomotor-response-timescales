...
if __name__ == "__main__":
    ...
    if not os.path.isdir('results/MS-222/fast/'):
        os.makedirs('results/MS-222/fast/')
    for fish_num in tqdm.trange(len(ms222_traces), desc='Fish no.'):
        ...
        if not os.path.isdir('results/MS-222/fast/'+ms222_traces[fish_num]):
            os.makedirs('results/MS-222/fast/'+ms222_traces[fish_num])
        ...
        for i in tqdm.trange(num_ics, desc='IC no.'):
            ...
            sio.savemat('results/MS-222/fast/'+ms222_traces[fish_num]+'/nonlinear'+str(i+1)+'.mat', {'c':c, 'cost':cost, 'grad':grad})
            cs_nonlin[i,:] = c
            costs_nonlin[i] = cost
        ...
        sio.savemat('results/MS-222/fast/'+ms222_traces[fish_num]+'.mat', {'c':c_linear, 'cost_f':cost_f_linear, 'cost_p':cost_p_linear, 'grad':grad_linear})
