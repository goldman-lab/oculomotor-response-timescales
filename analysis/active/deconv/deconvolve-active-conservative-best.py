...
if __name__ == "__main__":
    ...
    best_num_components_sr = {'090711e':3, '090811c':3, '090811d':4, '091111a':4, '091111c':3, '091211a':3}

    if not os.path.isdir('results'):
        os.makedirs('results')
    if not os.path.isdir('results/conservative'):
        os.makedirs('results/conservative')
    if not os.path.isdir('results/conservative/distributed'):
        os.makedirs('results/conservative/distributed')
    ...
    for fish_num in tqdm.trange(len(file_names), desc='Fish'):
        ...
        if not os.path.isdir('results/conservative/distributed/'+fish_name):
            os.makedirs('results/conservative/distributed/'+fish_name)
        ...
        for i in range(len(file_names[fish_num])):
            tranges[i], eye_pos[i], release_index = prepareStepResponse(file_names[fish_num][i], conservative=True, num_components_sr=n)
        ...
        for i in tqdm.trange(num_ics, desc='IC no.'):
            ...
            sio.savemat('results/conservative/distributed/'+fish_name+'/nonlinear'+str(i+1)+'.mat', {'c':c, 'cost':cost, 'grad':grad})
            cs_nonlin[i,:] = c
            costs_nonlin[i] = cost
        ...
        sio.savemat('results/conservative/distributed/'+fish_name+'.mat', {'c':c_linear, 'cost_f':cost_f_linear, 'cost_p':cost_p_linear, 'grad':grad_linear})
