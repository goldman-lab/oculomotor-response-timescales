...
if __name__ == "__main__":
    ...
    if not os.path.isdir('results/conservative/fast'):
        os.makedirs('results/conservative/fast')
    ...
    for fish_num in tqdm.trange(len(file_names), desc='Fish'):
        ...
        if not os.path.isdir('results/conservative/fast/'+fish_name):
            os.makedirs('results/conservative/fast/'+fish_name)
        ...
        for i in tqdm.trange(num_ics, desc='IC no.'):
            ...
            sio.savemat('results/conservative/fast/'+fish_name+'/nonlinear'+str(i+1)+'.mat', {'c':c, 'cost':cost, 'grad':grad})
            cs_nonlin[i,:] = c
            costs_nonlin[i] = cost
        ...
        sio.savemat('results/conservative/fast/'+fish_name+'.mat', {'c':c_linear, 'cost_f':cost_f_linear, 'cost_p':cost_p_linear, 'grad':grad_linear})
