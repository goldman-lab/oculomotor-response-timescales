...
if __name__ == "__main__":
    ...
    for fish_num in tqdm.trange(len(file_names), desc='Fish no.'):
        ...
        if not os.path.isdir('results/conservative/'+fish_name):
            os.makedirs('results/conservative/'+fish_name)
        if not os.path.isdir('results/conservative/'+fish_name+'/conservative_3tau'):
            os.makedirs('results/conservative/'+fish_name+'/conservative_3tau')
        n = 3 # best_num_components[fish_name]

        min_len = np.inf
        sr_fits = np.zeros((len(file_names[fish_num]), n*2))
        models = [[],]*len(file_names[fish_num])
        tranges = [[],]*len(file_names[fish_num])
        ...
        for trace_num in range(len(file_names[fish_num])):
            trange, data, release_index = prepareStepResponse(file_names[fish_num][trace_num],
                                                num_components_extrap = best_num_components_extrap[file_names[fish_num][trace_num]], conservative=True,
                                                full=False) # NB release_index is chosen as *exclusive* index for slicing in deconvolution procedure!

            # Make sure generated traces are same length as used for fitting
            # if len(trange) < min_len:
            #    min_len = len(trange)
            # get best fit
            fit_file = sio.loadmat('../fit/results/conservative/'+fish_name+'_conservative.mat')
            lls_fit = fit_file['lls']
        ...
        # sr_trange = trange[release_index-1:min_len]-trange[release_index-1]
        # min_len = len(sr_trange)
        # bootstrap_master = np.vstack([models[i][:min_len] for i in range(len(file_names[fish_num]))])
        ...
        for i in tqdm.trange(num_bootstrap_examples,desc='Bootstrap no.',leave=False):
            ...
            # bootstrap_noise = np.multiply(np.sqrt(samp_vars), np.random.standard_normal((len(file_names[fish_num]), min_len-1)).T)
            # bootstrap_data = np.copy(bootstrap_master)
            # bootstrap_data[:, 1:] += bootstrap_noise.T

            fits, lls, sses = fitting_functions.fitNEyePositions(tranges, bootstrap_data, min_num_components=n, max_num_components=n, num_ics=num_ics, inverse_tau_max = 1/(3*72*2e-4))
            sio.savemat('results/conservative/'+fish_name+'/conservative_3tau/bootstrap_'+str(i+1)+'.mat', {'fits': fits, 'sses':sses, 'lls':lls}, appendmat=False)
            for j in range(len(file_names[fish_num])):
                bootstrap_fits[i,j,:] = fits[n-1,j][np.argmax(lls[n-1,:]), :]

        # save overall results
        sio.savemat('results/conservative/'+fish_name+'_conservative_3tau.mat', {'fits':bootstrap_fits}, appendmat=False)
